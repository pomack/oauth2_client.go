...
func (p *mockOAuthClient) SetCurrentCredentials(value AuthToken) {
    p.currentCredentials = value.(MockAuthToken)
}
...
func (p *mockOAuthClient) SetAuthorizedResourceProtected(value bool) {
    p.authorizedResourceProtected = value
}
...
func (p *mockOAuthClient) GenerateRequestTokenUrl(properties jsonhelper.JSONObject) string {
    return p.requestTokenUrl
}
